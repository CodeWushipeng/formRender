...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      flowHandler() {
        this.listData = grid.busdata.list;
        // this.flowDialogTableVisible = true;
      },
      userHandler() {
        let res = [];
        for (let key in grid.busdata.user) {
          // console.log('key',key,FG.user[key])
          const value = grid.busdata.user[key];
          res.push({
            key: key,
            value: typeof  value == "object" ? JSON.stringify(value) : value
          })
        }
        this.userData = grid.busdata.user;
        this.userDataSolve = res;
      },
      platformHandler() {
        let res = [];
        for (let key in grid.busdata.platform) {
          const value = grid.busdata.platform[key]
          res.push({
            key: key,
            value: typeof  value == "object" ? JSON.stringify(value) : value
          })
        }
        this.platformData = grid.busdata.platform;
        this.platformDataSolve = res;
      },
      indataHandler(){
        let res = [];
        for (let key in grid.busdata.indata) {
          const value = grid.busdata.indata[key]
          res.push({
            key: key,
            value: typeof  value == "object" ? JSON.stringify(value) : value
          })
        }
        this.indata = grid.busdata.indata;
