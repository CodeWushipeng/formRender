      ...
      >
        <el-input size="mini" v-model="data.remoteCode">
          <template slot="append">
            <el-button icon="el-icon-search" @click="getData(data.remoteCode)"
              >检索</el-button
            >
          </template>
        </el-input>
...
import { getDicNoPage } from "../api/forms";
export default {
  components: {
    Draggable,
  },
      ...
      casProps: {
        ...
        lazyLoad(node, resolve) {
          ...
          if (node.level>0) {
            ...
            postData = {
              dicName: node.data ? node.data.name : "",
              selType: 2,
            };
            getDicNoPage({
              body: postData,
              ...
              .then((res) => {
                ...
                if (res.header && res.header.rspCode == RES_OK) {
                  if (res.body) {
                    ...
                    // this.total = res.body.dics.total
                    // this.pageSize = res.body.dics.size
                    let originData = res.body.dicList;
                    let nodes = originData.map((item) => ({
                      label: item.itemValue,
                      value: item.itemCode,
                      name: node.name,
